...
Below are some notes on how to build Stonkscoin Core for Windows.

The options known to work for building Stonkscoin Core on Windows are:

* On Linux using the [Mingw-w64](https://mingw-w64.org/doku.php) cross compiler tool chain. Ubuntu Trusty 14.04 is recommended
and is the platform used to build the Stonkscoin Core Windows release binaries.
...
for Ubuntu Xenial does not produce working executables for some of the Stonkscoin Core applications.
...
Note that for WSL the Stonkscoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/stonkscoin, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
This means you cannot use a directory that located directly on the host Windows file system to perform the build.

The next three steps are an example of how to acquire the source in an appropriate way.

    cd /usr/src
    sudo git clone https://github.com/stonkscoin-project/stonkscoin.git
    sudo chmod -R a+rw stonkscoin
...
way. This will install to `c:\workspace\stonkscoin`, for example:

    make install DESTDIR=/mnt/c/workspace/stonkscoin
...
causes two of the stonkscoin executables to crash shortly after start up. The bug is related to the
...
It's not possible to build the stonkscoin code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the stonkscoin source code).
