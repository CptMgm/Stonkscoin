...
#if defined(NDEBUG)
# error "StonksCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xc77a73bb30476e0d1db019b1830cc685aee0ba2196a01f029a931aa19fbaee46");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // StonksCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "StonksCoin Signed Message:\n";
...
{
    // StonksCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 694200); // StonksCoin: 420 69 420 Coins total 

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 2.5 * 24 * 60 * 60; // StonksCoin: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // StonksCoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // StonksCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // StonksCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x73c9c26848b814a52d09b6fa97077b73351491d45c9cf1fd6d9acc94fbfc417a");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "24/03/2021 - Elon Musk announces Teslas can be bought with cryptocurrency ";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("045309f056c039745e4581fa4fdffcb6ecf6f82861adef88d3413bcacf1d4fcb9091c7a4fdd1363352db996f3581ffebc2cd6c2a8f1cb7ce2a655a0d2e9b7884b5") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1616887489;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084971944;

        if (fTestNet)
        {
            block.nTime    = 1616887442;
            block.nNonce   = 386731287;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xbafde2c0b548b16a19d8ffbc7d9fe3db9fb462e56d0e69aa94f2ec56ae691348"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf3, 0xc3, 0xba, 0xd7 }; // StonksCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("StonksCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("StonksCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static StonksCoinMiner(CWallet *pwallet)
{
    printf("StonksCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("StonksCoin-miner");
    ...
    try { loop {
        ...
        printf("Running StonksCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("StonksCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&StonksCoinMiner, pwallet));
}
